...
def GetHtml():
    """
    Return the Html fragment from the clipboard or None if there is no Html
    in the clipboard.
    """
    cb = HtmlClipboard()
    if cb.HasHtmlFormat():
...
class HtmlClipboard:
    ...
    DEFAULT_HTML_BODY = \
        ...
        "<HTML><HEAD></HEAD><BODY><!--StartFragment-->%s<!--EndFragment-->"\
        "</BODY></HTML>"
    ...
    def GetFragment(self, refresh=False):
        """
        Return the Html fragment. A fragment is well-formated HTML
        enclosing the selected text
        """
    ...
    def GetSelection(self, refresh=False):
        """
        Return the part of the HTML that was selected.
    ...
    def PutToClipboard(self, html, fragmentStart, fragmentEnd,
                       selectionStart, selectionEnd, source="None"):
        """
        ...
        try:
            ...
            src = self.EncodeClipboardSource(html, fragmentStart, fragmentEnd,
                                             selectionStart, selectionEnd,
                                             source)
            # print(src)
            # print(self.GetCfHtml())
            win32clipboard.SetClipboardData(self.GetCfHtml(),
                                            src.encode("utf-8"))
    ...
    def EncodeClipboardSource(self, html, fragmentStart, fragmentEnd,
                              selectionStart, selectionEnd, source):
        """
        Join all our bits of information into a string formatted as per
        the HTML format specs.
        """
...
def DumpHtml():
    ...
    if cb.HasHtmlFormat():
        cb.GetFromClipboard()
        print("prefix=>>>{}<<<END".format(cb.prefix))
        print("htmlClipboardVersion=>>>{}<<<END".format(
            cb.htmlClipboardVersion))
        print("GetSelection()=>>>{}<<<END".format(cb.GetSelection()))
        print("GetFragment()=>>>{}<<<END".format(cb.GetFragment()))
        print("GetHtml()=>>>{}<<<END".format(cb.GetHtml()))
        print("GetSource()=>>>{}<<<END".format(cb.GetSource()))
...
if __name__ == '__main__':
    ...
    #     data = ("<p>Writing to the clipboard is <strong>easy"
    #             "</strong> with this code.</p>")
    #     PutHtml(data)
    #     if GetHtml() == data:
    #         print("passed")
    #     else:
